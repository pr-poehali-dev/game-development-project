...
type GameState = 'menu' | 'arrival' | 'door-dialogue' | 'house' | 'inspection' | 'dialogue' | 'kcs' | 'journal' | 'window' | 'naked-guest' | 'death' | 'alone' | 'player-infected' | 'self-check' | 'tv' | 'guest-mode' | 'guest-story' | 'guest-door' | 'guest-inside' | 'guest-caught' | 'guest-victory';
...
const Index = () => {
  ...
  const [guestModeDay, setGuestModeDay] = useState(1);
  const [guestSuspicion, setGuestSuspicion] = useState(0);
  const [guestStoryStep, setGuestStoryStep] = useState(0);
  const [guestChoice, setGuestChoice] = useState('');
  const [guestHouseName, setGuestHouseName] = useState('');
  const [guestDaysInside, setGuestDaysInside] = useState(0);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
            ...
            <div className="space-y-4 max-w-md mx-auto px-4">
              ...
              <Button 
                onClick={() => {
                  ...
                  setGuestSuspicion(0);
                  setGuestStoryStep(0);
                  setGuestDaysInside(0);
                  setGameState('guest-story');
                }}
                variant="default"
                className="w-full border-2 bg-destructive hover:bg-destructive/90"
                size="lg"
              >
                🎭 Играть за Гостя
              </Button>
      ...
      {gameState === 'guest-story' && (
        <div className="min-h-screen p-4 bg-gradient-to-b from-background to-destructive/20">
          <div className="max-w-4xl mx-auto flex items-center justify-center min-h-screen">
            <Card className="p-8 bg-card border-2 animate-fade-in">
              <div className="space-y-6">
                {guestStoryStep === 0 && (
                  <>
                    <div className="text-center space-y-4">
                      ...
                      <h2 className="text-3xl font-bold">Режим: Гость</h2>
                      <p className="text-lg text-muted-foreground">
                        Ты обычный человек. Город замерзает.
                      </p>
                    </div>
                    <div className="bg-secondary/30 p-6 rounded-lg space-y-3">
                      <p className="text-foreground">
                        Третий день эпидемии. Температура -40°C.
                      </p>
                      <p className="text-foreground">
                        Электричество отключили. Батареи холодные.
                      </p>
                      <p className="text-destructive font-bold">
                        Если не найдёшь тёплое убежище — замёрзнешь к утру.
                      </p>
                    </div>
                    <Button 
                      onClick={() => {
                ...
                {guestStoryStep === 1 && (
                  <>
                    <div className="text-center space-y-4">
                      ...
                      <h2 className="text-2xl font-bold">Твой дом разрушен</h2>
                    </div>
                    <div className="bg-secondary/30 p-6 rounded-lg space-y-3">
                      <p className="text-foreground">
                        Стена обвалилась. Ветер пронизывает насквозь.
                      </p>
                      <p className="text-foreground">
                        Соседи уже ушли искать убежище. Или мертвы.
                      </p>
                      <p className="text-muted-foreground text-sm italic">
                        "Нужно стучаться в дома и просить укрытия..."
                      </p>
                    </div>
                    <Button 
                      onClick={() => {
                ...
                {guestStoryStep === 2 && (
                  <>
                    <div className="text-center space-y-4">
                      <div className="text-6xl">❄️</div>
                      <h2 className="text-2xl font-bold">На улице</h2>
                    </div>
                    <div className="bg-secondary/30 p-6 rounded-lg space-y-3">
                      <p className="text-foreground">
                        Ледяной ветер режет лицо. Пальцы уже не чувствуешь.
                      </p>
                      <p className="text-foreground">
                        Впереди — дом с горящим светом. Там внутри тепло...
                      </p>
                      <p className="text-destructive font-bold">
                        Но тебя могут не впустить. Проверить. Застрелить.
                      </p>
                    </div>
                    <div className="space-y-3">
                      <Button 
                        onClick={() => {
                          setGuestChoice('honest');
                          setGameState('guest-door');
                          playSound('knock');
                        }}
                        className="w-full"
                        size="lg"
        ...
        <div className="min-h-screen p-4">
          <div className="max-w-4xl mx-auto space-y-6 pt-8">
            <div className="flex justify-between items-center">
              ...
              <Badge variant={guestSuspicion > 50 ? "destructive" : "default"} className="text-lg px-4 py-2">
                🔍 Подозрение: {guestSuspicion}%
              </Badge>
            </div>
            ...
            <Card className="p-8 bg-card border-2 animate-fade-in">
              <div className="space-y-6">
                <div className="text-center space-y-4">
                  <div className="text-6xl">🚪</div>
                  <h2 className="text-3xl font-bold">Дверь дома</h2>
                  <p className="text-muted-foreground italic">
                    *ТУК-ТУК-ТУК*
                  </p>
                </div>
                ...
                <div className="bg-secondary/30 p-6 rounded-lg space-y-4">
                  <p className="text-foreground font-bold">
                    Голос изнутри: "Кто там?!"
                  </p>
                  
                  {guestChoice === 'honest' && (
                    <div className="space-y-2">
                      <p className="text-foreground">
                        Ты: "Меня зовут... мне нужно укрытие. Мой дом разрушен. Я замерзаю."
                      </p>
                      <p className="text-muted-foreground text-sm">
                        *Пауза. Слышно шёпот за дверью...*
                      </p>
                      <p className="text-foreground">
                        Голос: "Откуда ты? Как тебя зовут по-настоящему?"
                      </p>
                    </div>
                  )}
                  ...
                  {guestChoice === 'lie' && (
                    <div className="space-y-2">
                      <p className="text-foreground">
                        Ты: "Я врач из больницы! У меня вакцина! Откройте быстрее!"
                      </p>
                      <p className="text-muted-foreground text-sm">
                        *Долгая тишина...*
                      </p>
                      <p className="text-destructive">
                        Голос: "Врач? Какой врач ходит без сумки в -40? Ты кто на самом деле?!"
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-destructive/20 border border-destructive/50 p-4 rounded-lg">
                  <p className="text-sm text-muted-foreground">
                    ...
                    Твой ответ повлияет на подозрения
                  </p>
                </div>

                <div className="space-y-3">
                  <Textarea
                    placeholder="Что ответишь? (Напиши свой текст)"
                    value={userMessage}
                    onChange={(e) => setUserMessage(e.target.value)}
                    rows={3}
                  ...
                  <Button 
                    onClick={() => {
                      ...
                      const suspicionAdd = guestChoice === 'lie' ? 30 : 15;
                      const nervousWords = ['бля', 'пожалуйста', 'умоляю', 'замерзаю', 'помогите'];
                      const hasNervous = nervousWords.some(w => userMessage.toLowerCase().includes(w));
                      
                      setGuestSuspicion(prev => Math.min(100, prev + suspicionAdd + (hasNervous ? 10 : 0)));
                      setUserMessage('');
                      
                      const houses = ['Семья Ивановых', 'Дом №7', 'Квартира Пётра', 'Убежище Марины', 'Дом старика Олега'];
                      setGuestHouseName(houses[Math.floor(Math.random() * houses.length)]);
                      
                      setGameState('guest-inside');
                      playSound('door');
                    }}
        ...
        <div className="min-h-screen p-4">
          <div className="max-w-4xl mx-auto space-y-6 pt-8">
            <div className="flex justify-between items-center">
              ...
              <Badge variant={guestSuspicion > 70 ? "destructive" : guestSuspicion > 40 ? "secondary" : "default"} className="text-lg px-4 py-2">
                🔍 Подозрение: {guestSuspicion}%
              </Badge>
            </div>
            ...
            <Card className="p-8 bg-card border-2 animate-fade-in">
              <div className="space-y-6">
                <div className="text-center space-y-4">
                  <div className="text-6xl">🔥</div>
                  <h2 className="text-3xl font-bold">Тебя впустили</h2>
                  <p className="text-muted-foreground">
                    Внутри тепло. Пальцы начинают оттаивать...
                  </p>
                </div>
                ...
                <div className="bg-secondary/30 p-6 rounded-lg space-y-4">
                  {guestSuspicion < 40 && (
                    <>
                      <p className="text-foreground">
                        Хозяин дома смотрит на тебя настороженно, но не агрессивно.
                      </p>
                      <p className="text-foreground">
                        — Можешь остаться. Но я буду следить. Первое подозрительное движение — вылетишь.
                      </p>
                      <p className="text-muted-foreground text-sm italic">
                        "Главное — вести себя нормально. Не привлекать внимания..."
                      </p>
                    </>
                  )}

                  {guestSuspicion >= 40 && guestSuspicion < 70 && (
                    <>
                      <p className="text-foreground">
                        Хозяин смотрит на тебя с явным недоверием.
                      </p>
                      <p className="text-destructive">
                        — Что-то в тебе не то. Я буду проверять каждый день. Один странный жест — и пуля в лоб.
                      </p>
                      <p className="text-muted-foreground text-sm italic">
                        "Чёрт... нужно быть осторожнее. Любая ошибка = смерть."
                      </p>
                    </>
                  )}
                  ...
                  {guestSuspicion >= 70 && (
                    <>
                      <p className="text-destructive font-bold">
                        Хозяин достаёт пистолет и направляет на тебя.
                      </p>
                      <p className="text-destructive">
                        — Ты врёшь. Я вижу. Стой где стоишь. Сейчас я тебя проверю.
                      </p>
                      <p className="text-muted-foreground text-sm italic">
                        "Пиздец... надо что-то придумать БЫСТРО..."
                      </p>
                    </>
                  )}
                </div>

                <Alert className={guestSuspicion >= 70 ? "border-destructive bg-destructive/10" : "border-primary/50"}>
                  <Icon name="Target" className="h-4 w-4" />
                  <AlertDescription>
                    <strong>Твоя цель:</strong> Выжить {5 - guestDaysInside} дней, не вызывая подозрений
                  </AlertDescription>
                </Alert>
                ...
                {guestSuspicion >= 70 ? (
                  <div className="space-y-3">
                    <Button 
                      onClick={() => {
                        if (Math.random() > 0.3) {
                          setGameState('guest-caught');
                          playSound('gunshot');
                        } else {
                          setGuestSuspicion(50);
                          setGuestDaysInside(prev => prev + 1);
                          if (guestDaysInside >= 4) {
                            setGameState('guest-victory');
                          } else {
                            setGuestModeDay(prev => prev + 1);
                            setGameState('guest-inside');
                          }
                          playSound('click');
                        }
                  ...
                  <Button 
                    onClick={() => {
                      ...
                      if (guestDaysInside >= 4) {
                        ...
                        if (randomEvent > 0.7) {
                          setGuestSuspicion(prev => Math.min(100, prev + 20));
                        }
                        playSound('footstep');
                      }
                    }}
                    ...
                    size="lg"
                  >
                    Дожить до утра (День {guestDaysInside + 1})
                  </Button>
                )}
              </div>
            </Card>
              ...
              <div className="space-y-8 text-center">
                ...
                <div className="space-y-6">
                  ...
                  <p className="text-2xl text-foreground">
                    Тебя убили как подозреваемого
                  </p>
                  <p className="text-lg text-muted-foreground">
                    Прожил в доме: {guestDaysInside} {guestDaysInside === 1 ? 'день' : 'дней'}
                  </p>
                </div>
                <Button 
                  onClick={() => setGameState('menu')}
      ...
      {gameState === 'guest-victory' && (
        <div className="min-h-screen p-4 bg-gradient-to-b from-background to-primary/20">
          <div className="max-w-4xl mx-auto flex items-center justify-center min-h-screen">
            <Card className="p-12 bg-card border-4 border-primary">
              <div className="space-y-8 text-center">
                <div className="text-9xl">🎉</div>
                <div className="space-y-6">
                  <p className="text-4xl text-primary font-bold animate-fade-in">
                    ТЫ ВЫЖИЛ!
                  </p>
                  <p className="text-2xl text-foreground">
                    5 дней в убежище
                  ...
                  <p className="text-lg text-muted-foreground">
                    Эпидемия пошла на спад. Власти начали эвакуацию.
                  </p>
                  <p className="text-md text-muted-foreground italic">
                    "Главное — остаться человеком... в прямом смысле."
                  </p>
                </div>
                <Button 
                  onClick={() => setGameState('menu')}
